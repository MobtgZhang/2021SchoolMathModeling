...
def create_filter_documents(save_json, saved_filter_file, stop_words_file, jieba=True, singleWords=False):
    documents = []
    # 使用处理好之后的文件
    with open(save_json, mode="r", encoding="utf-8") as rfp:
        raw_data = json.load(rfp)
        for item in tqdm(raw_data, desc='get document'):
            article = item['article']
            idx = item['id']
            sentence = filter_chinese(article)
            if sentence != '':
                if jieba:
                    document = jieba_segment(sentence, stop_words_file, singleWords)
                documents.append([idx, document])
        with open(saved_filter_file, mode="w", encoding="utf-8") as wfp:
            json.dump(documents, wfp)
